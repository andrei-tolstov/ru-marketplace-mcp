...
- `drom_card_photos(url_or_id, max_photos=5, high_res=True)` — downloads and returns binary images (`ImageContent`) and lot summary for multimodal inspection.
...
## Обязательный визуальный анализ деталей по фото

В классифайдах автозапчастей (особенно контрактных и б/у) текст объявления часто шаблонный или не отражает реальных повреждений. При любом подборе или проверке запчасти модель **ОБЯЗАНА** вызывать `drom_card_photos` и выполнять экспертную визуальную инспекцию:

1. **Проверка соответствия запчасти (Точно ли эта деталь?):**
   - **Геометрия и форма:** сравнить форму корпуса, ребер жесткости и монтажных плоскостей с эталоном.
   - **Сторона и ось:** строго проверять ориентацию — левая (LH) или правая (RH), передняя или задняя.
   - **Рестайлинг / поколение:** сверять форму оптики, бамперов, решеток, зеркал и расположение крепежей (дорестайл часто несовместим с рестайлом).
   - **Электрика и разъемы:** проверять форму разъемов (фишек), количество пинов (контактов) и тип датчиков/моторчиков.
   - **Маркировка и OEM:** искать на фото выштамповки, заводские гравировки и оригинальные наклейки с каталожными номерами.

2. **Оценка физического и технического состояния:**
   - **Крепежные элементы:** убедиться в целостности всех монтажных ушек, кронштейнов, защелок и направляющих (отломанные крепежи фар и бамперов — частый скрытый дефект).
   - **Целостность корпуса:** осмотр на предмет микротрещин, сколов, заломов, глубоких царапин на видовых или рабочих поверхностях.
   - **Следы ремонта:** выявлять следы пайки пластика, впаянную металлическую сетку, кустарную сварку, следы эпоксидки или неоригинального герметика.
   - **Коррозия и окисление:** проверять металл на глубокую ржавчину, а разъемы и клеммы — на зеленый/белый налет окислов.
   - **Геометрия и правка:** следы рихтовки, вытягивания, заломы силовых элементов кузова и подвески.
   - **Резинотехнические изделия:** рассыхание, надрывы и трещины пыльников, сальников, сайлентблоков.

3. **Вердикт пользователю:**
   - Четко зафиксировать: найдена именно запрашиваемая запчасть или есть отличия по модификации.
   - Детально описать состояние детали по фото и перечислить все замеченные дефекты.
   - Дать обоснованную рекомендацию: "Деталь в отличном состоянии, можно брать" / "Есть незначительные косметические дефекты" / "Критический износ/сломаны крепления — брать не рекомендуется".
